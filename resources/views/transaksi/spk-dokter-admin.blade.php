...
@section('content')
    ...
    <div class="page-wrapper">
        ...
        <div class="content container-fluid">
            ...
            <!-- Search Filter -->
            <form action="{{ route('spk-dokter-admin-cari') }}" method="GET" id="search-form">
                @csrf
                ...
                <div class="col-md-12">
                    <div class="table-responsive">
                        <table class="table table-striped custom-table datatable">
                            ...
                            <tbody>
                                @foreach ($data_spk_dokter as $sqlspk => $result_spk_dokter)
                                    <tr>
                                        {{-- <td>{{ ++$sqlspk }}</td> --}}
                                        <td class="id">{{ $result_spk_dokter->id }}</td>
                                        <td class="name">{{ $result_spk_dokter->name }}</td>
                                        <td class="nip">{{ $result_spk_dokter->nip }}</td>
                                        <td class="unit_kerja">{{ $result_spk_dokter->unit_kerja }}</td>
                                        <td class="nomor_sip">{{ $result_spk_dokter->nomor_sip }}</td>
                                        <td class="tanggal_terbit">{{ $result_spk_dokter->tanggal_terbit }}</td>
                                        <td class="tanggal_berlaku">{{ $result_spk_dokter->tanggal_berlaku }}</td>
                                        <td class="sip_spk_jabatan">{{ $result_spk_dokter->sip_spk_jabatan }}</td>
                                        <td class="jenis_dokumen">{{ $result_spk_dokter->jenis_dokumen }}</td>
                                        <td class="ruangan">{{ $result_spk_dokter->ruangan }}</td>
                                        <td class="dokumen_sip"><a href="{{ asset('assets/DokumenSPKDokter/' . $result_spk_dokter->dokumen_sip) }}" target="_blank"></a></td>
                                        ...
                                        <td class="text-right">
                                            <div class="dropdown dropdown-action">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    ...
                                                    <a class="dropdown-item delete_spk_dokter" href="#" data-toggle="modal" data-target="#delete_spk_dokter"><i class="fa fa-trash-o m-r-5"></i>Delete</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <div class="modal-body">
                        <form action="{{ route('transaksi/spk-dokter/tambah-data') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <input type="hidden" class="form-control" name="jenis_dokumen" value="SPK Dokter" readonly>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Ruangan</label>
                                        <br>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="theSelect" name="ruangan" id="e_ruangan" style="width: 100% !important">
                                            <option selected disabled>-- Pilih Ruangan --</option>
                                            @foreach($ruanganOptions as $key => $value)
                                                @if (!empty($result_spk_dokter->ruangan))
                                                    <option value="{{ $key }}" {{ $key == $result_spk_dokter->ruangan ? 'selected' : '' }}>{{ $value }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    </div>
                ...
                <div class="modal-content">
                    <div class="modal-body">
                        <div class="form-header">
                            <h3>Hapus SPK Dokter</h3>
                            <p>Apakah anda yakin ingin menghapus data ini?</p>
                        </div>
                        <div class="modal-btn delete-action">
                            <form action="{{ route('transaksi/spk-dokter/hapus-data') }}" method="POST">
                                @csrf
                                <input type="hidden" name="id" class="e_id" value="">
                                <input type="hidden" name="dokumen_sip" class="d_dokumen_sip_dokter" value="">
                                <div class="row">
                                    <div class="col-6">
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                ...
                $('.dokumen_sip a').each(function() {
                    ...
                    @if (!empty($result_spk_dokter->dokumen_sip))
                        $(this).closest('td').after('<td hidden class="dokumen_sip">{{ $result_spk_dokter->dokumen_sip }}</td>');
                    @endif
                });
            });
        </script>
