...
function handleFileChange(input) {
    ...
    const filePreview3 = dropzoneArea.querySelector(".message-preview3");
    const filePreview4 = dropzoneArea.querySelector(".message-preview4");

    if (input.files.length > 0) {
        const file = input.files[0];
        if (input.id === "dokumen_kk") {
            filePreview1.innerHTML = `${file.name}, ${file.size} bytes`;
        } else if (input.id === "dokumen_akta_lahir_anak") {
            filePreview2.innerHTML = `${file.name}, ${file.size} bytes`;
        } else if (input.id === "pas_foto_ayah") {
            filePreview3.innerHTML = `${file.name}, ${file.size} bytes`;
        } else if (input.id === "pas_foto_ibu") {
            filePreview4.innerHTML = `${file.name}, ${file.size} bytes`;
        }
    } else {
        filePreview1.innerHTML = "Tidak ada file yang di pilih";
        filePreview2.innerHTML = "Tidak ada file yang di pilih";
        filePreview3.innerHTML = "Tidak ada file yang di pilih";
        filePreview4.innerHTML = "Tidak ada file yang di pilih";
    }
}
